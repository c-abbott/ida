...
\vspace{-10mm}
\begin{align*}
  Q(\boldsymbol{\beta}|\boldsymbol{\beta^{(t)}}) &= E_{\boldsymbol{y_{mis}}}\left[l(\boldsymbol{\beta})|\boldsymbol{y_{obs}}, \boldsymbol{x}, \boldsymbol{\beta^{(t)}}\right]\\
  &=\sum_{i=1}^m\left\{y_i(\beta_0 + \beta_1x_i)\right\} - \sum_{i=1}^n\log(1+e^{\beta_0 + \beta_1x_i}) + 
  \sum_{i=m+1}^n(\beta_0 + \beta_1x_i) E_{\boldsymbol{y_{mis}}}[y_i|\boldsymbol{y_{obs}}, \boldsymbol{x}, \boldsymbol{\beta^{(t)}}]\\
  &=\sum_{i=1}^m\left\{y_i(\beta_0 + \beta_1x_i)\right\} - \sum_{i=1}^n\log(1+e^{\beta_0 + \beta_1x_i}) + 
  \sum_{i=m+1}^n(\beta_0 + \beta_1x_i)p_i(\boldsymbol{\beta})
\end{align*}

* Where we have used the result that $E[Y_i] = p_i(\boldsymbol{\beta})$ since $Y_i \overset{\text{ind}}{\sim} \text{Bernoulli}\{(p_i(\boldsymbol{\beta})\}$. We remind the reader of the definition of  $p_i(\boldsymbol{\beta})$
