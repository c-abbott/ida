...
The missing data resulting from this mechanism will hence be MNAR indicating that MDM (b)
is **NOT** ignorable for likelihood-based inference.
...
* The R code below presents our results where we yield values of $\widehat{\beta_0}_{\text{MLE}} =  0.9755$ to 4 d.p and $\widehat{\beta_1}_{\text{MLE}} =  -2.4804$ to 4 d.p as our MLEs for $\boldsymbol{\beta} = (\beta_0, \beta_1)'$.
...
* Using the code defined below, we find the MLEs for each component of $\boldsymbol{\theta}$ to be (4 d.p):
...
# Define density function observed in the question
mixture_density = function(yy, p, mu, sigma2, lambda) {
  p*dlnorm(yy, mu, sqrt(sigma2)) + (1-p)*dexp(yy, lambda)
}

# Plotting
data = data.frame(dataex5)
ggplot(data, aes(x = dataex5)) +
  geom_histogram(aes(y = stat(density), fill = 'Histogram of dataex5')) +
  geom_function(aes(color='Estimated Density'),
                fun = mixture_density, args=list(p=p, mu=mu, sigma2=sigma2, lambda=lambda),
                size=1.5) +
  scale_x_continuous(name = "y", limits=c(0, 110)) + 
  scale_y_continuous(name = "Probability Density", limits=c(0, 0.065)) +
  ggtitle("Histogram of dataex5 and estimated density") + 
  theme_bw() +
